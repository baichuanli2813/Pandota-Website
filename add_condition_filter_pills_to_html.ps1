...
Write-Host "Rebuilding inventory.html with Search by Condition Filter Pills..."
...
foreach ($item in $items) {
    ...
    # Precise Item Condition Rules:
    $exactCond = "Used"
    $condIcon = "fa-rotate-left"
    
    if ($itemId -eq "267755930402") {
        # HP Omen MAX 16 is Opened - never used
        $exactCond = "Opened - never used"
        $condIcon = "fa-box-open"
    } elseif ($itemId -eq "267727293251") {
        # Lenovo Legion Go 2 is BRAND NEW
        $exactCond = "New"
        $condIcon = "fa-sparkles"
    } elseif ($titleRaw -match 'FAULTY|\*FAULTY\*') {
        $exactCond = "For parts or not working"
        $condIcon = "fa-wrench"
    } elseif ($titleRaw -match 'Open Box|Opened|Unused') {
        $exactCond = "Opened - never used"
        $condIcon = "fa-box-open"
    } elseif ($titleRaw -match '^\s*NEW\b|NEW SEALED|SEALED') {
        $exactCond = "New"
        $condIcon = "fa-sparkles"
    } else {
        $exactCond = "Used"
      ...
      <div class="container">
        <div class="section-title-area" style="margin-bottom: 30px;">
          ...
          <p class="section-desc">
            Browse all 82 active listings currently available on Pandota's official eBay store. Filter by condition or search below!
          </p>
        </div>
...
Write-Host "Successfully rebuilt inventory.html with Condition Filter Pills!"
