...
Write-Host "Applying strict Opened-never used condition rules for all "$items.Count" items..."
...
foreach ($item in $items) {
    ...
    # Accurate eBay Condition Rule:
    # 1) If title says "SEALED" => New (Sealed)
    # 2) If title says "NEW" or "Open Box" or "Opened" => Opened - never used
    # 3) If title says "FAULTY" => For parts or not working
    # 4) Otherwise => Used
    
    $cond = "Used"
    $condIcon = "fa-rotate-left"
    
    if ($titleRaw -match 'SEALED') {
        $cond = "New (Sealed)"
        $condIcon = "fa-sparkles"
    } elseif ($titleRaw -match '^\s*NEW\b|Open Box|Opened|Unused') {
...
Write-Host "Successfully built inventory.html with Opened - never used condition pills!"
