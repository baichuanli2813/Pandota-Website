...
Write-Host "Rendering all "$items.Count" items with 100% exact scraped prices..."
...
foreach ($item in $items) {
    ...
    if ($item.Title -match '(Ultra 9|Ultra 7|Ultra 5|i9|i7|i5|Ryzen 9|Ryzen 7|Ryzen AI|Snapdragon X)') {
        $cpu = $matches[1]
    } elseif ($item.Title -match 'Lens|Sony|Camera') {
...
Write-Host "Successfully built inventory.html with ALL 82 EXACT SCRAPED PRICES!"
