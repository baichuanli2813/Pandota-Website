...
Write-Host "Applying user's exact eBay condition mapping for all "$items.Count" items..."
...
foreach ($item in $items) {
    ...
    # User-Specified Exact Mapping Rule:
    # 1) "For parts or not working" for FAULTY items
    # 2) "Opened - never used" for Open Box / Opened items
    # 3) "New" for NEW items
    # 4) "Used" for pre-owned items
    ...
    # 1. Exact eBay Item Condition Pill
...
Write-Host "Successfully built inventory.html with user's exact condition mapping!"
