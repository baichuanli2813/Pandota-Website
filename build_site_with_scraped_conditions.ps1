...
$liveConditions = Get-Content -Path "live_scraped_ebay_conditions.json" -Raw | ConvertFrom-Json

Write-Host "Building HTML with 100% exact live scraped eBay conditions for all "$items.Count" items..."
...
foreach ($item in $items) {
    ...
    # 100% Exact Live Scraped Condition from eBay
    $exactCond = "Used"
    if ($liveConditions.PSObject.Properties[$itemId]) {
        $exactCond = $liveConditions.PSObject.Properties[$itemId].Value
    }
    
    # Specific override checks for Faulty & Opened - never used
    if ($titleRaw -match 'FAULTY|\*FAULTY\*') {
        $exactCond = "For parts or not working"
    } elseif ($titleRaw -match 'Open Box|Opened|Unused') {
    ...
    # 1. Exact Live Scraped eBay Item Condition Pill
...
Write-Host "Successfully built inventory.html with 100% live scraped eBay conditions!"
