...
Write-Host "Updating HP Omen MAX and Open Box items to Opened - never used..."
...
foreach ($item in $items) {
    ...
    # Exact eBay Condition Mapping:
    $exactCond = "Used"
    $condIcon = "fa-rotate-left"
    
    # Explicit Overrides
    if ($itemId -eq "267755930402") {
        # HP Omen MAX 16 is Opened - never used on eBay
        ...
        $exactCond = "Opened - never used"
        $condIcon = "fa-box-open"
    } elseif ($titleRaw -match 'SEALED') {
        $exactCond = "New"
        $condIcon = "fa-sparkles"
    } elseif ($titleRaw -match '^\s*NEW\b') {
        # Items starting with NEW that are unsealed / open box returns
        $exactCond = "Opened - never used"
        $condIcon = "fa-box-open"
    } else {
        $exactCond = "Used"
...
Write-Host "Successfully updated HP Omen MAX to Opened - never used!"
