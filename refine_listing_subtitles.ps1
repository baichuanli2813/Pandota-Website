...
Write-Host "Refining subtitles for all "$items.Count" store listings..."
...
foreach ($item in $items) {
    ...
    # Refined spec extractors
    $cpuPill = ""
    $gpuPill = ""
    $ramStoragePill = ""
    
    if ($titleRaw -match 'Lens|Camera|SEL\d|Body|A6700|A7') {
        # Camera / Optics category
        if ($titleRaw -match '(26MP|33MP|24\.2 MP|Mirrorless|Full-Frame|APS-C)') {
            $cpuPill = $matches[1] + " System"
        } else {
            $cpuPill = "Optical Imaging"
        }
        
        if ($titleRaw -match '(\d+-\d+mm|\d+mm)') {
            $gpuPill = $matches[1] + " Focal Spec"
        } elseif ($titleRaw -match 'F/\d+\.\d|f/\d+\.\d') {
            $gpuPill = $matches[0] + " Fast Aperture"
        } else {
            $gpuPill = "E-Mount Precision"
        }
        
        if ($titleRaw -match 'Warranty|WTY') {
            $ramStoragePill = "Pandota Store Warranty"
        } else {
            $ramStoragePill = "DPD Next-Day Express"
        }
    } else {
        # Laptop / Workstation category
        # 1. CPU
        if ($titleRaw -match '(Ultra 9 \d+HX|Ultra 9 \d+H|Ultra 7 \d+HX|Ultra 7 \d+H|Ultra 5 \d+V|Ultra 5 \d+U|i9 \d+HX|i9 \d+H|i7 \d+HX|i7 \d+H|i7 \d+U|Ryzen AI 9 HX \d+|Ryzen 9 \d+HX|Ryzen 7 \d+HS|Snapdragon X2 Elite|Snapdragon X)') {
            $cpuPill = $matches[1]
        } elseif ($titleRaw -match '(Ultra 9|Ultra 7|Ultra 5|i9|i7|i5|Ryzen 9|Ryzen 7|Ryzen AI)') {
            $cpuPill = $matches[1] + " Processor"
        } else {
            $cpuPill = "Intel / AMD High-Performance CPU"
        }
        
        # 2. GPU
        if ($titleRaw -match '(RTX 5090|RTX 5080|RTX 5070 Ti|RTX 5070|RTX 5060|RTX 4090|RTX 4080|RTX 4070|RTX 4060|RTX 4050|RTX 3080 Ti|RTX 3070 Ti|RTX 3050|RTX 2000 Ada|RTX PRO 4000|RTX PRO 3000|RTX PRO 2000|RTX PRO 500|RTX A2000)') {
            $gpuPill = "NVIDIA " + $matches[1]
        } elseif ($titleRaw -match 'Intel Graphics|Iris Xe') {
            $gpuPill = "Intel Ultra Graphics"
        } else {
            $gpuPill = "Dedicated Graphics"
        }
        
        # 3. Memory & Display / Feature Pill
        $ram = ""
        $storage = ""
        $display = ""
        ...
        if ($titleRaw -match '(\d+TB SSD|\d+TB|\d+GB SSD)') {
            $storage = $matches[1]
        }
        if ($titleRaw -match '(300Hz|250Hz|240Hz|180Hz|165Hz|144Hz|120Hz|OLED|4K|QHD\+|Mini LED|3\.2K|2\.8K)') {
            $display = $matches[1]
        }
        
        if ($ram -and $storage) {
            $ramStoragePill = "$ram &bull; $storage"
        } elseif ($ram -and $display) {
            $ramStoragePill = "$ram &bull; $display"
        } elseif ($display) {
            $ramStoragePill = "$display Display"
        } else {
            $ramStoragePill = "DPD Next-Day Delivery"
        }
    }
          ...
          <div class="inventory-card" data-price="$numPrice">
            ...
            <div class="inv-card-body">
              ...
              <div class="inv-card-features">
                <span><i class="fa-solid fa-microchip"></i> $cpuPill</span>
                <span><i class="fa-solid fa-compact-disc"></i> $gpuPill</span>
                <span><i class="fa-solid fa-memory"></i> $ramStoragePill</span>
              </div>
...
Write-Host "Successfully built inventory.html with REFINED SPEC SUBTITLES!"
