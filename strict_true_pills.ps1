...
Write-Host "Building strictly true spec pills for all "$items.Count" listings..."
...
foreach ($item in $items) {
    ...
    # 1. CPU / Processor / Sensor / Core Spec
    ...
    # 4. Condition / Warranty if explicitly in title
    if ($titleRaw -match '(NEW SEALED|NEW|Body Only|Faulty|\*FAULTY\*)') {
        $pills += "<span><i class=`"fa-solid fa-tag`"></i> " + $matches[1] + "</span>"
    }
...
Write-Host "Successfully built inventory.html with strictly true spec pills!"
